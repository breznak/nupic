...
def _buildArgs(f, self=None, kwargs={}):
  ...
  if kwargs.has_key('numActiveColumnsPerInhArea'):
    setattr(self, 'numActiveColumnsPerInhArea', kwargs['numActiveColumnsPerInhArea'])
    kwargs.pop('numActiveColumnsPerInhArea')
    
  if kwargs.has_key('potentialPct'):
    setattr(self, 'potentialPct', kwargs['potentialPct'])
    kwargs.pop('potentialPct')
...
class SPRegion(PyRegion):
  ...
  def _allocateSpatialFDR(self, rfInput):
    ...
    if ( (self.SpatialClass == CPPSpatialPooler) or
      ...
      autoArgs.pop('columnDimensions')
      autoArgs.pop('inputDimensions')
      autoArgs.pop('potentialRadius')
      autoArgs.pop('cloneMap')
      autoArgs.pop('numCloneMasters')
  
      columnDimensions = (self.columnCount, 1)
      inputDimensions = (1, self.inputWidth)
      potentialRadius = inputDimensions[1]/2
  
      cloneMap, numCloneMasters = fdru.makeCloneMap(
          columnsShape=columnDimensions,
          outputCloningWidth=columnDimensions[1],
          outputCloningHeight=columnDimensions[0]
        )
